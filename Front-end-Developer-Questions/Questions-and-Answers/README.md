...
- xml、html、xhtml之间的关系？
   ...
   xml：可扩展标记语言，是一种跨平台语言，可自由创建标签，其前身是SGML（标准通用标记语言）。

   html：超文本标记语言，大小写混写且编码不规范，在手机等相对不太智能的浏览器上无法合理地解析不规范的html文档。

   xhtml：对html进行规范，编码更加规范严谨，可以看成是html的一个子集，是一种html向xml过渡的语言。相对html而言，xhtml必须小写、标签必须嵌套、闭合，同时不支持属性最小化，例：<input checked/>需要写成<input checked="checked"/>

   三者对比：xml设计用来描述数据，重点在什么是数据以及如何存放数据，html是用来显示数据的，重点在怎么显示数据上面。html结合了xml的规则之后诞生了xhtml。**XML是SGML的子集，html基于SGML，xhtml基于XML，可以说html5结合了html和xml。**

   总结：html是最原始的网页设计语言，因为当时很多html规范都是先实行后有标准的，所以导致当时的绝大多数html页面都是有错误的，随后w3c为了规范html，就用xml的规则来约束html，于是就制定了xhtml。w3c的初衷是对xhtml这个MIME type浏览器要执行强错误检查，页面有错误就要显示错误信息，但是当时很多存在的页面大部分都是有错的，这个方案无法推行，故w3c在标准之后增加了一个附录，表示允许开发者用xhtml的语法来写页面，用旧的的MIME type来分发页面，这个旧的MIME type 不会触发浏览器进行强错误检查。但是在之后的xhtml1.1和2中w3c把这个附录去掉，导致很少有人去执行这个标准，到后来html5标准的制定，就将向后兼容作为一个重要的原则，现在任何页面把第一行改成<!DOCTYPE html>都是可以的。
